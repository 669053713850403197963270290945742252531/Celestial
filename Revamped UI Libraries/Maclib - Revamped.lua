...
local function createTooltip(tooltipText)
	if MacLib.TooltipsEnabled then
		local tooltip = Instance.new("Frame", GetGui())
		tooltip.Name = "Tooltip"
		tooltip.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
		tooltip.BackgroundTransparency = 0.2
		tooltip.BorderSizePixel = 0
		tooltip.ZIndex = 1000
		tooltip.AutomaticSize = Enum.AutomaticSize.XY -- Allow dynamic sizing
	
		-- Label
	
		local tooltipLabel = Instance.new("TextLabel", tooltip)
		tooltipLabel.Name = "tooltipLabel"
		tooltipLabel.FontFace = Font.new(assets.interFont)
		tooltipLabel.RichText = true
		tooltipLabel.TextWrapped = true
		tooltipLabel.AutomaticSize = Enum.AutomaticSize.XY
		tooltipLabel.Text = tostring(tooltipText)
		tooltipLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		tooltipLabel.TextTransparency = 0.5
		...
		local uiPadding = Instance.new("UIPadding", tooltipLabel)
		...
		local function updatePosition()
			while tooltip.Parent do
				...
				local tooltipWidth = tooltip.AbsoluteSize.X
				local tooltipHeight = tooltip.AbsoluteSize.Y
				...
				local newX = math.clamp(mousePos.X + offsetX, 0, screenSize.X - tooltipWidth - 10)
				local newY = math.clamp(mousePos.Y + offsetY, 0, screenSize.Y - tooltipHeight - 10)
...
function MacLib:Window(Settings)
	...
	if Settings.GlobalSettings == nil then
		Settings.GlobalSettings = true
	end
	...
	globalSettingsButton.Visible = Settings.GlobalSettings
...
do
	...
	function WindowFunctions:GlobalSetting(Settings)
		...
		local tooltipConfig = Settings.Tooltip or false

		local function setupTooltip(globalSetting)
			if not globalSetting then return end -- Global setting check
		
			local tooltipObject
		
			globalSetting.MouseEnter:Connect(function()
				if not tooltipObject then
					tooltipObject = createTooltip(tooltipConfig)
				end
			end)
		
			globalSetting.MouseLeave:Connect(function()
		...
		if tooltipConfig and typeof(tooltipConfig) == "string" then

			setupTooltip(globalSetting)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			local isDisabled = Settings.Disabled or false

			tabSwitcher.Interactable = not isDisabled

			if isDisabled then
				tabSwitcherName.TextColor3 = Color3.new(255, 0, 0)
				tabImage.ImageColor3 = Color3.new(255, 0, 0)
			end

			--[[
			buttonInteract.TextTransparency = isDisabled and 0.7 or 0.5
			buttonImage.ImageTransparency = isDisabled and 0.7 or 0.5

			button.Interactable = not isDisabled
			buttonInteract.Interactable = not isDisabled
			buttonImage.Interactable = not isDisabled
			
			if isDisabled then
				buttonInteract.TextColor3 = Color3.new(255, 0, 0)
				buttonImage.ImageColor3 = Color3.new(255, 0, 0)
			end
			]]
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					local isDisabled = ButtonFunctions.Settings.Disabled or false
					...
					local function setupTooltip(button)
						if not button then return end -- Button check
					...
					function ButtonFunctions:SetVisible(State)
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					local isDisabled = ToggleFunctions.Settings.Disabled or false

					toggleName.TextTransparency = isDisabled and 0.7 or 0.5
					toggle1.ImageTransparency = isDisabled and 0.7 or 0.5

					toggle.Interactable = not isDisabled
					toggleName.Interactable = not isDisabled
					toggle1.Interactable = not isDisabled
					togglerHead.Interactable = not isDisabled

					if isDisabled then
						toggleName.TextColor3 = Color3.fromRGB(255, 0, 0)
						toggle1.ImageColor3 = Color3.fromRGB(181, 27, 27)
						togglerHead.ImageColor3 = Color3.fromRGB(255, 0, 0)
					end

					-- "Tooltip" configuration

					local tooltipConfig = ToggleFunctions.Settings.Tooltip or false

					local function setupTooltip(toggle)
						if not toggle then return end -- Toggle check
					...
					function ToggleFunctions:SetVisible(State)
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					local isDisabled = SliderFunctions.Settings.Disabled or false

					slider.Interactable = not isDisabled

					if isDisabled then
						sliderName.TextTransparency = 0.7
						sliderName.TextColor3 = Color3.fromRGB(255, 0, 0)

						sliderValue.TextTransparency = 0.2
						sliderValue.TextColor3 = Color3.fromRGB(255, 0, 0)
						sliderValue.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
						sliderValueUIStroke.Color = Color3.fromRGB(255, 0, 0)

						sliderBar.ImageColor3 = Color3.fromRGB(255, 0, 0)
						sliderBar.ImageTransparency = 0.6

						sliderHead.ImageColor3 = Color3.fromRGB(255, 0, 0)
					end
					
					-- "CustomValue" configuration

					local CustomValue = SliderFunctions.Settings.CustomValue or false
					...
					local tooltipConfig = SliderFunctions.Settings.Tooltip or false
			
					local function setupTooltip(sliderName)
						if not sliderName then return end -- Slider check
					...
					if tooltipConfig and typeof(tooltipConfig) == "string" then
						...
						setupTooltip(sliderName)
			
					end

					-- Fixing errors when invalid content is inputted
					...
					local function SetValue(val, ignorecallback)
						...
						-- if RoundedValue is enabled, round the value to the specified Precision

						local RoundedValue = SliderFunctions.Settings.RoundedValue

						if RoundedValue then
							local precision = SliderFunctions.Settings.Precision or 0

							finalValue = tonumber(string.format("%." .. precision .. "f", finalValue)) -- Round to the specified precision
						end
					...
					sliderValue.FocusLost:Connect(function(enterPressed)
						...
						if not number then
							sliderValue.Text = SliderFunctions.Settings.Default
							SetValue(SliderFunctions.Settings.Default)
							return
						end
						...
						if value then
							...
							-- "LimitInput" configuration

							local LimitInput = SliderFunctions.Settings.LimitInput

							local givenValue = value
							local sliderMin = SliderFunctions.Settings.Minimum
							local sliderMax = SliderFunctions.Settings.Maximum
							
							local maxHeadPos = UDim2.new(1, 0, 0.5, 0)
							local minHeadPos = UDim2.new(0, 0, 0.5, 0)
							
							if typeof(LimitInput) == "table" then
								local adjustedValue = givenValue
								local headPosition = nil
							
								-- "Greater" in LimitInput table, allow values greater than the max

								if table.find(LimitInput, "Greater") and givenValue > sliderMax then
									--print("Input values are allowed to be GREATER than the maximum")
									adjustedValue = givenValue -- Allow the value to exceed the max
									headPosition = maxHeadPos -- Restrict slider head to end
								end
							
								-- "Less" in LimitInput table, allow values less than the min

								if table.find(LimitInput, "Less") and givenValue < sliderMin then
									--print("Input values are allowed to be LESS than the minimum")
									adjustedValue = givenValue -- Allow the value to go below min
									headPosition = minHeadPos -- Restrict slider head to beginning
								end
							
								-- If "Greater" and "Less" are NOT in LimitInput table, clamp the value and slider head position

								if not table.find(LimitInput, "Greater") and givenValue > sliderMax then
									--print("Value exceeds maximum, clamping to maximum")
									adjustedValue = sliderMax
									headPosition = maxHeadPos -- Set to ending
								elseif not table.find(LimitInput, "Less") and givenValue < sliderMin then
									--print("Value below minimum, clamping to minimum")
									adjustedValue = sliderMin
									headPosition = minHeadPos -- Set to beginning
								end
							
								SetValue(adjustedValue)

								if headPosition then
									sliderHead.Position = headPosition -- Update slider head position
								end

							else
								--print("LimitInput is true, enforcing strict limits")
								
								local clampedValue = math.clamp(givenValue, sliderMin, sliderMax)
					...
					function SliderFunctions:SetVisible(State)
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					local isDisabled = InputFunctions.Settings.Disabled or false

					input.Interactable = not isDisabled

					if isDisabled then
						inputName.TextTransparency = 0.7
						inputName.TextColor3 = Color3.fromRGB(255, 0, 0)
						...
						inputBoxUIStroke.Color = Color3.fromRGB(255, 0, 0)
					end
					
					-- "Tooltip" configuration

					local tooltipConfig = InputFunctions.Settings.Tooltip or false	
			
					local function setupTooltip(inputName)
						if not inputName then return end -- Button check
					
						local tooltipObject
					
						inputName.MouseEnter:Connect(function()
							if not tooltipObject then
								tooltipObject = createTooltip(tooltipConfig)
							end
						end)
					
						inputName.MouseLeave:Connect(function()
							if tooltipObject then
								tooltipObject:Destroy()
								tooltipObject = nil
							end
					...
					function InputFunctions:SetVisible(State)
						input.Visible = State
					end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					local isDisabled = KeybindFunctions.Settings.Disabled or false

					keybind.Interactable = not isDisabled

					if isDisabled then
						keybindName.TextTransparency = 0.7
						keybindName.TextColor3 = Color3.fromRGB(255, 0, 0)

						binderBox.TextTransparency = 0.2
						binderBox.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
						binderBox.TextColor3 = Color3.fromRGB(255, 0, 0)

						binderBoxUIStroke.Color = Color3.fromRGB(255, 0, 0)
					end

					-- "Tooltip" configuration

					local tooltipConfig = KeybindFunctions.Settings.Tooltip or false	
			
					local function setupTooltip(keybindName)
						if not keybindName then return end -- Button check
					
						local tooltipObject
					
						keybindName.MouseEnter:Connect(function()
							if not tooltipObject then
								tooltipObject = createTooltip(tooltipConfig)
							end
						end)
					
						keybindName.MouseLeave:Connect(function()
							if tooltipObject then
								tooltipObject:Destroy()
								tooltipObject = nil
							end
					...
					UserInputService.InputBegan:Connect(function(inp)
						if focused and not isBinding then
							...
							Event = UserInputService.InputBegan:Connect(function(input)
								local BlacklistedBinds = KeybindFunctions.Settings.BlacklistedBinds or {}
							
								-- Check if given keycode is blacklisted BEFORE declaring it binded

								if table.find(BlacklistedBinds, input.KeyCode) or table.find(BlacklistedBinds, input.UserInputType) then
									--warn(tostring(input.KeyCode.Name or input.UserInputType.Name) .. " is an invalid keybind.")
									
									-- Reset to default keybind

									binded = KeybindFunctions.Settings.Default
									binderBox.Text = binded and binded.Name or "..." -- If no default keybind defined, fallback to "..."
									WindowFunctions:SetKeybind(binded) -- Set window keybind to default keybind as a safety measure to prevent window keybind malfunction

									WindowFunctions:Notify({
										Title = "Interface",
										Description = input.KeyCode.Name .. " is an invalid keybind."
									})
								...
								-- If the key isn't blacklisted, allow the bind
								...
								-- Trigger binded callback ONLY if set
					...
					function KeybindFunctions:SetVisible(State)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					local isDisabled = DropdownFunctions.Settings.Disabled or false

					dropdown.Interactable = not isDisabled

					if isDisabled then
						dropdown.BackgroundTransparency = 0.9
						dropdown.BackgroundColor3 = Color3.fromRGB(255, 0, 0)

						dropdownName.TextTransparency = 0.7
						dropdownName.TextColor3 = Color3.fromRGB(255, 0, 0)

						dropdownImage.ImageTransparency = 0.6
						dropdownImage.ImageColor3 = Color3.fromRGB(255, 0, 0)

						dropdownUIStroke.Color = Color3.fromRGB(255, 0, 0)
						dropdownUIStroke.Transparency = 0.85
					end
					...
					local function setupTooltip(interact)
						if not interact then return end -- Button check
					...
					function DropdownFunctions:SetVisible(State)
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					local AlphaEnabled = ColorpickerFunctions.Settings.AlphaEnabled

					if AlphaEnabled == nil then
						AlphaEnabled = true
					end
					...
					local isDisabled = ColorpickerFunctions.Settings.Disabled or false

					colorpicker.Interactable = not isDisabled

					if isDisabled then
						colorpickerName.TextTransparency = 0.7
						colorpickerName.TextColor3 = Color3.fromRGB(255, 0, 0)
					end

					-- "Tooltip" configuration

					local tooltipConfig = ColorpickerFunctions.Settings.Tooltip or false
			
					local function setupTooltip(colorpickerName)
						if not colorpickerName then return end -- Button check
					
						local tooltipObject
					
						colorpickerName.MouseEnter:Connect(function()
							if not tooltipObject then
								tooltipObject = createTooltip(tooltipConfig)
							end
						end)
					
						colorpickerName.MouseLeave:Connect(function()
							if tooltipObject then
								tooltipObject:Destroy()
								tooltipObject = nil
							end
					...
					function ColorpickerFunctions:SetVisible(State)
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					function HeaderFunctions:SetVisible(State)
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					function LabelFunctions:SetVisible(State)
						label.Visible = State
					end
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					function SubLabelFunctions:SetVisible(State)
				...
				function SectionFunctions:Paragraph(Settings, Flag)
					...
					function ParagraphFunctions:SetVisible(State)
				...
				function SectionFunctions:Divider()
					...
					function DividerFunctions:SetVisible(State)
				...
				function SectionFunctions:Spacer()
					...
					function SpacerFunctions:SetVisible(State)
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local scriptOptionsSection = TabFunctions:Section({ Side = "Right" })
                ...
                configSection:Button({
					...
					Callback = function()
                        if not selectedConfig or selectedConfig == "" then
                            WindowFunctions:Notify({
                                Title = "Error",
                                Description = "No configuration selected to delete."
                            })
                            return
                        else
                            WindowFunctions:Dialog({
                                Title = "Delete Config",
                                Description = "Are you sure you want to delete " .. selectedConfig .. "? This action is irreversible.",
                                Buttons = {
                                    {
                                        Name = "Confirm",
                                        Callback = function()
                                            
                                            local success, returned = MacLib:DeleteConfig(selectedConfig)
                                            if not success then
                                                WindowFunctions:Notify({
                                                    Title = "Interface",
                                                    Description = "Unable to delete config, return error: " .. returned
                                                })
                                            else
                                                WindowFunctions:Notify({
                                                    Title = "Interface",
                                                ...
                                                configSelection:ClearOptions()
                                                configSelection:InsertOptions(MacLib:RefreshConfigList())
                                                configSelection:UpdateSelection(0)
                                            end
				...
				configSection:Button({
					...
					Callback = function()
                        if not selectedConfig or selectedConfig == "" then
                            WindowFunctions:Notify({
                                Title = "Error",
                                Description = "No configuration selected to overwrite."
                            })
                            return
                        else
                            WindowFunctions:Dialog({
                                Title = "Overwrite Config",
                                Description = "Are you sure you want to overwrite " .. selectedConfig .. "? This action will overwrite " .. selectedConfig .. " with your current configurations and is irreversible.",
                                Buttons = {
                                    {
                                        Name = "Confirm",
                                        Callback = function()
				...
				configSection:Button({
					...
					Callback = function()
						...
						WindowFunctions:Notify({
							Title = "Interface",
							Description = string.format("Set %q to autoload", name),
						})
					end,
				})
                ...
                configSection:Button({
					Name = "Reset autoload",
					Callback = function()
						local autoloadFile = isfile(MacLib.Folder .. "/settings/autoload.txt")
						local autoloadContents

						if not autoloadFile then
							WindowFunctions:Notify({
								Title = "Interface",
								Description = "No available autoload.",
							})
							return
						else
							autoloadFile = MacLib.Folder .. "/settings/autoload.txt"
							autoloadContents = readfile(autoloadFile)
						end

						WindowFunctions:Dialog({
							Title = "Reset autoload",
							Description = "Are you sure you want to reset your current autoload? This action will delete autoload '" .. autoloadContents .. "'.",
							Buttons = {
								{
									Name = "Confirm",
									Callback = function()
				...
				if isfile(MacLib.Folder .. "/settings/autoload.txt") then
					...
					autoloadLabel:UpdateName("Autoload config: " .. name or "none")
				end

				-- Section: Options

				scriptOptionsSection:Toggle({
					Name = "Tooltips",
					Default = true,
					Callback = function(value)
						MacLib.TooltipsEnabled = value
					end,
				}, "toggleTooltips")
			end
