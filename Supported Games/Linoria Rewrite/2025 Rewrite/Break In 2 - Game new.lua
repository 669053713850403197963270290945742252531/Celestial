...
godmodeDepbox:AddSlider(
	"godmodeDelay_Slider",
	{ Text = "Delay", Tooltip = false, Default = 0.05, Min = 0, Max = 10, Rounding = 2 }
)
godmodeDepbox:AddToggle(
	"godmodeHideEnergyGain_Toggle",
	{ Text = "Hide Energy Gain", Tooltip = 'Hides the "+x" text above the energy bar while keeping the "-x" text.' }
)

exploitsGroup1:AddToggle("noclip_Toggle", { Text = "Noclip", Tooltip = false, Default = false }):AddKeyPicker(
	"noclip_KeyPicker",
	{ Default = "G", SyncToggleState = true, Mode = "Toggle", Text = "Noclip", NoUI = false }
)

exploitsGroup1
	:AddToggle("fly_Toggle", { Text = "Fly", Tooltip = false, Default = false })
	:AddKeyPicker(
		"fly_KeyPicker",
...
flyDepbox:AddSlider("flyHorizontalSpeed_Slider", {
	Text = "Horizontal Speed",
	Tooltip = "How fast you fly in all directions.",
	Default = 50,
	Min = 10,
	Max = 300,
	Rounding = 1,
})
flyDepbox:AddSlider("flyVerticalSpeed_Slider", {
	Text = "Vertical Speed",
	Tooltip = "How fast you fly ascending/descending.",
	Default = 30,
	Min = 5,
	Max = 150,
	Rounding = 1,
...
speedExploitDepbox:AddSlider(
	"speedExploitDelay_Slider",
	{ Text = "Delay", Tooltip = false, Default = 1, Min = 0, Max = 10, Rounding = 2 }
)
speedExploitDepbox:AddToggle("speedExploitHideTrail_Toggle", { Text = "Hide Trail (Client-Sided)", Tooltip = false })

exploitsGroup1:AddToggle(
	"collectCash_Toggle",
	{ Text = "Collect Cash", Tooltip = "Collects all on-ground cash drops." }
)
...
autoKillDepbox:AddSlider(
	"autoKillDelay_Slider",
	{ Text = "Damage Amount", Tooltip = false, Default = 100, Min = 1, Max = 100, Rounding = 0 }
)
...
exploitsGroup2:AddToggle(
	"hiddenItemESP_Toggle",
	{ Text = "Hidden Item ESP", Tooltip = "Highlights all the items inside drawers." }
)

local hiddenItemsDepbox = exploitsGroup2:AddDependencyBox()

hiddenItemsDepbox:AddLabel("ESP Color"):AddColorPicker(
	"hiddenItemESPColor_Colorpicker",
	{ Title = "ESP Color", Default = Color3.fromRGB(252, 186, 3), Transparency = 0.5 }
)
...
hiddenItemESPLabelDepbox:AddLabel("Label Color"):AddColorPicker(
	"hiddenItemLabelColor_Colorpicker",
	{ Title = "Label Color", Default = Color3.fromRGB(36, 173, 26), Transparency = false }
)
hiddenItemESPLabelDepbox:AddToggle("rainbowHiddenItemLabel_Toggle", { Text = "Rainbow Label", Tooltip = false })
hiddenItemESPLabelDepbox:AddSlider(
	"hiddenItemLabelSize_Slider",
	{ Text = "Label Size", Tooltip = false, Default = 15, Min = 10, Max = 30, Rounding = 0 }
)
...
badGuyESPDepbox:AddLabel("ESP Color"):AddColorPicker(
	"badGuyESPColor_Colorpicker",
	{ Title = "ESP Color", Default = Color3.fromRGB(255, 0, 0), Transparency = 0.5 }
)
...
exploitsGroup3:AddDropdown("locationTeleport_Dropdown", {
	Values = {
		"Villian Base",
		"Kitchen",
		"Fighting Arena",
...
exploitsGroup3:AddButton({
	Text = "Teleport",
	Tooltip = false,
	DoubleClick = false,
	Func = function()
		local cframeValues = {
			["Villian Base"] = CFrame.new(
				-233.926117,
				30.4567528,
				-790.019897,
				0.00195977557,
		...
		if getLib("entityLib").checkTeleport(hrp, cframeDestination, 5) then
			...
			elseif dropdown.Value == "Detective" then
				local clickDetector =
					game.Workspace.TheHouse.OfficeDoor.ClosedDoor.Handle:FindFirstChildOfClass("ClickDetector")
...
itemsGroup2:AddDropdown("itemList_Dropdown", {
	Values = {
		"GoldPizza",
		"GoldenApple",
		"RainbowPizzaBox",
...
itemsGroup2:AddSlider(
	"giveItemAmount_Slider",
	{ Text = "Amount", Tooltip = false, Default = 10, Min = 1, Max = 800, Rounding = 0 }
)

notify("erghregthehrteh", 10)

itemsGroup2:AddInput("customGiveAmount_Input", {
	Default = "",
	Numeric = true,
	Finished = false,
...
itemsGroup2:AddDropdown("weaponList_Dropdown", {
...
local giveWeapon_Btn = itemsGroup2:AddButton({
	Text = "Give Weapon",
	Tooltip = false,
	DoubleClick = false,
	Func = function()
...
local giveBestWeapon_Btn = itemsGroup2:AddButton({
...
menuGroup:AddToggle("keybindMenu_Toggle", {
	Default = linoria.KeybindFrame.Visible,
	Text = "Keybind Menu",
	Callback = function(enabled)
		linoria.KeybindFrame.Visible = enabled
	end,
})
menuGroup:AddToggle("customCursor_Toggle", {
	Text = "Custom Cursor",
	Default = useCustomCursor,
	Callback = function(enabled)
		linoria.ShowCustomCursor = enabled
	end,
})
menuGroup:AddToggle("autoResetRainbowColor_Toggle", {
	Text = "Auto Reset Color",
	Tooltip = "Resets the colorpicker color back to its default color once a rainbow toggle is disabled.",
	Default = true,
	Callback = function(enabled)
		window.autoResetRainbowColor = enabled
	end,
})
menuGroup:AddToggle("executeOnTeleport_Toggle", {
	Text = "Execute on Teleport",
	Tooltip = "Runs the script when a game teleport is detected. Once the script is queued, it cannot be unqueued.",
	Default = false,
})
menuGroup:AddToggle("notifSound_Toggle", {
	Text = "Notification Alert Sounds",
	Tooltip = false,
	Default = true,
	Callback = function(enabled)
		window.notifSoundEnabled = enabled
	end,
})

local notifSoundDepbox = menuGroup:AddDependencyBox()

notifSoundDepbox:AddSlider("notifAlertVolumeSlider", {
	Text = "Alert Volume",
	Tooltip = false,
	Default = 1,
	Min = 0.1,
...
menuGroup:AddDropdown("notifSide", {
	...
	Tooltip = "The side of the screen notifications will appear on.",
})
