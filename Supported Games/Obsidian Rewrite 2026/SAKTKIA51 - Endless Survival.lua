...
local function isPowerOn()
    local poxerBoxHandle = area51.ElectricityRoom.PowerManager.Interruptor.Part

    if poxerBoxHandle.CFrame == CFrame.new(121.500084, 323.400116, 599.499939, -1, 0, 0, 0, -1, 0, 0, 0, 1) then
        return true
    end
...
local function perkEnabled(perkName)
    local perks = localplayer.Perks

    if perks and perks:FindFirstChild(perkName) then
        return perks[perkName].Value
    end
...
local gunModFuncs = {
    infiniteAmmo = function()
        for _, weapon in next, getgc(true) do
            if type(weapon) == "table" and rawget(weapon, "ammo") then
                weapon.ammo = math.huge
                weapon.stored_ammo = math.huge
            end
        end
    end,
    ...
    fireRate = function()
        for _, weapon in next, getgc(true) do
            if type(weapon) == "table" and rawget(weapon, "ammo") then
                weapon.shoot_wait = 1e-14
                weapon.is_auto = true
            end
        end
    end,

    bulletCount = function(count)
        for _, weapon in next, getgc(true) do
            if type(weapon) == "table" and rawget(weapon, "ammo") then
                weapon.bullet_count = count
            end
        end
    end
}
...
teleportGroup:AddDropdown("regularLocationsList_Dropdown", { Text = "Teleport", Tooltip = false, Values = { "Area 51", "Teleporter", "Alien", "Alien Code Input",
"Pack A Punch Machine", "Ammo Box", "Armor", "Radioactive Zone" }, Default = 1 })
...
otherGroup2:AddToggle("weaponPromptReach_Toggle", { Text = "Weapon Prompt Reach", Tooltip = false, Default = false })
otherGroup2:AddToggle("instantInteract_Toggle", { Text = "Instant Interact", Tooltip = false, Default = false })
otherGroup2:AddToggle("autoPutupBoard_Toggle", { Text = "Auto Plank", Tooltip = "Automatically puts up all nearby planks.", Default = false })

powerEnable_Btn = otherGroup2:AddButton({ Text = "Turn on Power", Disabled = isPowerOn(), DisabledTooltip = "The power is already on.",
    Func = function()
        if not isPowerOn() then
            ...
            entityLib.teleport(CFrame.new(127.10587310791016, 323.5, 599.6689453125)) -- teleport to power box
            task.wait(0.2)
            utils.fireProxPrompt(area51.ElectricityRoom.PowerManager.Interruptor.Base)
            task.wait(0.03)
            entityLib.restoreData(hrp, "CFrame")
            repeat task.wait() until isPowerOn()
            notify("Success", "The power has been turned on.", 6)
            powerEnable_Btn:SetDisabled(true)
        else
            notify("Error", "The power is already on.", 6)
            powerEnable_Btn:SetDisabled(true)
        end
    end,
})
...
toggles.promptReach_Toggle:OnChanged(function(enabled) 
    for _, obj in ipairs(game:GetService("Workspace").Doors:GetDescendants()) do
...
toggles.weaponPromptReach_Toggle:OnChanged(function(enabled) 
    for _, obj in ipairs(game:GetService("Workspace").Weapons:GetDescendants()) do
        if obj:IsA("ProximityPrompt") then
            if enabled then
                entityLib.storeData(obj, "MaxActivationDistance", true)
...
toggles.autoPutupBoard_Toggle:OnChanged(function(enabled)
    ...
    -- Helper: Is it a "Killers Spawn" model?
    local function isKillersSpawnModel(model)
        return model:IsA("Model") and model.Name == "Killers Spawn"
    end

    -- Adds prompts inside the given model
    local function addPromptsFromModel(model)
        for _, prompt in ipairs(model:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") then
                cachedPrompts[prompt] = true
                --print("[autoPutupBoard] Added prompt:", prompt:GetFullName())
            end
        end
    end
    ...
    for _, descendant in ipairs(area51:GetDescendants()) do
        if isKillersSpawnModel(descendant) then
            addPromptsFromModel(descendant)
        end
    end

    -- Handle new "Killers Spawn" models
    table.insert(conns, area51.DescendantAdded:Connect(function(desc)
        if isKillersSpawnModel(desc) then
            addPromptsFromModel(desc)
        elseif desc:IsA("ProximityPrompt") and desc:IsDescendantOf(area51) then
            local parent = desc:FindFirstAncestor("Killers Spawn")
            if parent then
                cachedPrompts[desc] = true
                --print("[autoPutupBoard] Late prompt added:", desc:GetFullName())
            end
        end
    end))

    table.insert(conns, area51.DescendantRemoving:Connect(function(desc)
        if cachedPrompts[desc] then
            cachedPrompts[desc] = nil
            --print("[autoPutupBoard] Removed prompt:", desc:GetFullName())
        end
    end))

    -- Main loop
    task.spawn(function()
        --print("[autoPutupBoard] Loop started")
        while toggles.autoPutupBoard_Toggle.Value and activeLoop do
            for prompt in pairs(cachedPrompts) do
                if typeof(prompt) == "Instance" and prompt:IsDescendantOf(game) and prompt.Enabled and prompt.Parent then
        ...
        --print("[autoPutupBoard] Loop ended")
    end)
end)
...
othersGroup3:AddToggle("autoRevive_Toggle", { Text = "Auto Revive", Tooltip = "Automatically revives yourself with the Who's Who perk.", Default = false })

local autoReviveDepbox = othersGroup3:AddDependencyBox()

autoReviveDepbox:AddToggle("autoPurchaseWhosWho_Toggle", { Text = "Auto Purchase", Tooltip = "Automatically purchases the perk again if you can afford it.", Default = false })

autoReviveDepbox:SetupDependencies({ { toggles.autoRevive_Toggle, true } })
...
toggles.autoRevive_Toggle:OnChanged(function(enabled)
    if enabled then
        task.spawn(function()
            local awaitingRepurchase = false
            local revivedOnce = false

            while toggles.autoRevive_Toggle.Value do
                local hasPerk = perkEnabled("Who's Who")

                if not hasPerk then
                    awaitingRepurchase = true
                    revivedOnce = false

                    local targetName = "Original" .. localplayer.Name
                    local origChar = game:GetService("Workspace")["Characters to kill"]:FindFirstChild(targetName)

                    if origChar then
                        local hrp = origChar:FindFirstChild("HumanoidRootPart")
                        local prompt = hrp and hrp:FindFirstChild("RevivePrompt")

                        if hrp and prompt then
                            localplayer.Character:PivotTo(hrp.CFrame + Vector3.new(0, 3, 0))
                            utils.fireProxPrompt(prompt.Parent)
                ...
                if awaitingRepurchase and revivedOnce and isAlive() then
                    if toggles.autoPurchaseWhosWho_Toggle and toggles.autoPurchaseWhosWho_Toggle.Value then
                        local perkPrompt = game:GetService("Workspace").Perks["Who's Who"]:FindFirstChild("Range")
                        if perkPrompt then
                            task.wait(1)

                            if not perkEnabled("Who's Who") then
                                utils.fireProxPrompt(perkPrompt)
                            end
                        end
                    end

                    awaitingRepurchase = false
                    revivedOnce = false
                end
...
toggles.autoPurchaseWhosWho_Toggle:OnChanged(function(enabled)
    if enabled and not perkEnabled("Who's Who") then
        utils.fireProxPrompt(game:GetService("Workspace").Perks["Who's Who"].Range)
    end
end)
...
killerNametagDepbox:AddDivider()
...
fieldOfViewDepbox:AddDivider()

visualsGroup:AddToggle("antiInk_Toggle", { Text = "Anti Ink", Tooltip = "Disables the black ink popup from Eyeless Jack." })
visualsGroup:AddToggle("antiTailsPopup_Toggle", { Text = "Anti Tails Popup", Tooltip = "Disables the popup from Tails Doll." })
...
local function destroyInkEvent()
    local eyelessJack = killersFolder:FindFirstChild("Eyeless Jack")
    if eyelessJack then
        local event = eyelessJack:FindFirstChild("Inked")
        if event then
            event:Destroy()
        end
    end
...
local function disableInkGui()
    local inkGui = localplayer.PlayerGui:FindFirstChild("Ink")

    if inkGui then
        local isInked = inkGui:FindFirstChildWhichIsA("ImageLabel", true)
        if isInked and isInked.Visible then
            inkGui.Enabled = false
        end
    end
end

local killerConnection
toggles.antiInk_Toggle:OnChanged(function(enabled)
    if enabled then
        destroyInkEvent()
        disableInkGui()

        killerConnection = killersFolder.ChildAdded:Connect(function(child)
            if child.Name == "Eyeless Jack" then
                task.wait(0.1)
                destroyInkEvent()
            end
        end)
...
toggles.antiInk_Toggle:OnChanged(function(enabled)
    if enabled then
        task.spawn(function()
            while toggles.antiInk_Toggle.Value do
...
toggles.antiTailsPopup_Toggle:OnChanged(function(enabled)
    local popupGui = localplayer.PlayerGui:FindFirstChild("TailsPopup")

    if popupGui then
        popupGui.Enabled = not enabled
    end
end)
...
local weaponsGroup = tabs.exploits:AddLeftGroupbox("Weapons")
local upgradesGroup = tabs.exploits:AddRightGroupbox("Upgrades")
local othersGroup = tabs.exploits:AddLeftGroupbox("Other")
...
-- Replace this with your actual toggles and dropdown objects:
local dropdown = upgradesGroup:AddDropdown("availableUpgradesList_Dropdown", {
    Text = "Available Upgrades",
    Tooltip = false,
    Values = { "Carpenter", "Double Points", "Fire Sale", "Insta Kill", "Max Ammo", "Nuke" },
    Default = nil,
    Multi = true,
    Searchable = true,
    Callback = function(val)
        -- Clear and update the selected upgrades table
        for k in pairs(selectedUpgrades) do
            selectedUpgrades[k] = nil
        end
        for k, v in pairs(val) do
            selectedUpgrades[k] = v
        end
        print("[Dropdown] Selected upgrades:")
        for k,v in pairs(selectedUpgrades) do
            print(" ", k, v)
        end
    end,
})

local toggle = upgradesGroup:AddToggle("autoCollectUpgrades_Toggle", {
    Text = "Collect Upgrades",
    Tooltip = false,
})
...
-- Watch for toggle changes (poll or event-based depending on your UI lib)
toggle:OnChanged(function(enabled)
    print("[Toggle] Auto collect:", enabled)

    if enabled and not collecting then
        collecting = true
        collectTask = task.spawn(function()
            print("[Collector] Started collecting loop")
            while toggle.Value do
                local hrp = getHRP()
                if not hrp then
                    warn("[Collector] No HRP")
                    task.wait(0.5)
                    continue
                end

                for _, upgradeObj in ipairs(Workspace:GetChildren()) do
                    if selectedUpgrades[upgradeObj.Name] and upgradeObj:IsA("Model") then
                        local hitbox = upgradeObj:FindFirstChild("Hitbox")
                        if hitbox and hitbox:IsA("BasePart") then
                            print("[Collector] Touching", upgradeObj.Name)
                            fireTouch(hrp, hitbox)
                        else
                            warn("[Collector] No valid Hitbox for", upgradeObj.Name)
                        end
                    end
                end
            ...
            print("[Collector] Stopped collecting loop")
            collecting = false
        end)
    elseif not enabled and collecting then
        -- Loop will exit naturally because toggle.Value is false
        print("[Toggle] Disabling collecting loop")
    end
end)
...
othersGroup:AddButton({ Text = "Obtain Armor",
    Func = function()
        local char = localplayer.Character
        local armor = char:FindFirstChild("VestArmor")

        if armor then
            notify("Error", "You already have armor.", 10)
            return
        end
...
gunModsDepbox:SetupDependencies({ { toggles.enableGunMods_Toggle, true } })

toggles.infiniteAmmoGunMod_Toggle:OnChanged(function(enabled)
    if enabled then
        gunModFuncs.infiniteAmmo()
    end
end)
...
local function setupBackpackListener()
    ...
    backpack.ChildAdded:Connect(function(tool)
        if toggles.infiniteAmmoGunMod_Toggle.Value and tool:IsA("Tool") then
            task.delay(0.5, function()
                gunModFuncs.infiniteAmmo()
                gunModFuncs.fullAuto()
                gunModFuncs.fireRate()
                gunModFuncs.bulletCount(options.bulletCountGunMod_Slider.Value)
            end)
        end
    end)
end
...
localplayer.CharacterAdded:Connect(function()
    task.defer(setupBackpackListener)
end)

toggles.fullAutoGunMod_Toggle:OnChanged(function(enabled)
    if enabled then
        gunModFuncs.fullAuto()
    end
end)

toggles.fireRateGunMod_Toggle:OnChanged(function(enabled)
    if enabled then
        gunModFuncs.fireRate()
    end
end)

options.bulletCountGunMod_Slider:OnChanged(function(val)
    gunModFuncs.bulletCount(val)
end)
...
othersGroup2:AddButton({ Text = "Give Office Card",
    Func = function()
        if entityLib.getTool("Specific", "Card2") then
            notify("Error", "You already have the office keycard.", 8)
            return
        end
...
othersGroup2:AddButton({ Text = "Vault Keycard",
    Func = function()
        if entityLib.getTool("Specific", "Card1") then
            notify("Error", "You already have the vault keycard.", 8)
            return
        end

        local hrp = getHRP()
        utils.fireTouchEvent(hrp, area51.JailsRoom.Jails.Left.Behind.Card)
    end,
})
...
local function unloadModules()
	local modules = { "autoDrink_Toggle", "autoRefill_Toggle", "promptReach_Toggle", "weaponPromptReach_Toggle", "instantInteract_Toggle", "speedExploit_Toggle", "enableFly_Toggle", "toggleFly_Toggle",
    "enableNoclip_Toggle", "enableJumpPower_Toggle", "doorNoclip_Toggle", "removeFog_Toggle", "killerESP_Toggle", "toggleKillersNametags_Toggle", "enableFOV_Toggle",
    "removeBearTraps_Toggle", "disableRadioactiveZone_Toggle", "removeBarriers_Toggle", "removeEntrance_Toggle", "disableRadioactiveFog_Toggle", "disableKillParts_Toggle",
    "disableAllKillers_Toggle", "killerGodmode_Toggle", "antiPinhead_Toggle", "antiSlendermanJumpscare_Toggle", "rainbowESPColor_Toggle",
    "rainbowNameTagColor_Toggle", "antiInk_Toggle", "antiTailsPopup_Toggle", "autoPutupBoard_Toggle" }
...
saveManager:SetFolder("Celestial ScriptHub/SAKTKIA51")
saveManager:SetSubFolder("Endless Survival")
